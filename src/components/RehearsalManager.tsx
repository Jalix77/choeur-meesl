...
interface Props {
  ...
  initialProgramItems?: ServiceProgramItemWithProfile[]
  editMode?: boolean
}

type ChoristerSelection = { profile_id: string; vocal_role: VocalRole }

export default function RehearsalManager({ songs, choristers, rehearsal, initialChoristers, initialProgramItems, editMode }: Props) {
  ...
  // Programmation du culte — préchargée avec les 14 éléments par défaut si aucune n'existe encore
  const [programItems, setProgramItems] = useState<ProgramItemDraft[]>(
    initialProgramItems && initialProgramItems.length > 0
      ? [...initialProgramItems]
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (rehearsalId) {
      ...
      await supabase.from('service_program_items').delete().eq('rehearsal_id', rehearsalId)
      const validProgramItems = programItems.filter(it => it.label.trim() !== '')
      if (validProgramItems.length > 0) {
        ...
        if (programErr) {
          setError(`Erreur programmation du culte : ${programErr.message} (code: ${programErr.code})`)
          setLoading(false)
          return
        }
      }
      ...
      if (form.notify_whatsapp && servicePeople.length > 0) {
            ...
            <div className="mt-3 space-y-2 border-t border-[#E2B36A]/30 pt-3">
              ...
              <label className="flex items-center gap-2 cursor-pointer select-none">
                ...
                <span className="text-sm text-[#5A3318] flex items-center gap-1.5">
                  ...
                  Envoyer une notification email aux personnes en service (choristes + programmation)
                </span>
              </label>
              ...
              <label className="flex items-center gap-2 cursor-pointer select-none">
                ...
                <span className="text-sm text-[#5A3318] flex items-center gap-1.5">
                  ...
                  Envoyer une notification WhatsApp aux personnes en service (choristes + programmation)
                </span>
              </label>
            </div>
          </div>
